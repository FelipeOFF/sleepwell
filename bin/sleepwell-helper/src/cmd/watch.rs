...
pub fn run(args: WatchArgs) -> Result<()> {
    ...
    loop {
        // Poll for events with a short timeout so we can also check shutdown.
...
fn classify(e: &notify_debouncer_mini::DebouncedEvent) -> &'static str {
    ...
    // The mini debouncer only distinguishes Any vs AnyContinuous; we map
    // by checking if the path currently exists to infer create/modify/remove.
    match e.kind {
